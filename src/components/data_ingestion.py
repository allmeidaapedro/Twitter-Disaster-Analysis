...
class DataIngestionConfig:
    ...
    `train_data_path`, `test_data_path` and `raw_data_path` that specify the default paths for train, test and raw data files respectively.
...
class DataIngestion:
    ...
    def apply_data_ingestion(self):
        ...
        try:
            ...
            logging.info('Cleaning the data with nlp techniques.')
            
            clean_df = nlp_data_cleaning(df)

            logging.info('Obtaining X and y.')

            X = clean_df['clean_text'].copy()
            ...
            logging.info('Saving train and test sets into a csv.')
